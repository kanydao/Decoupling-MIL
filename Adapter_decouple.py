...
class Adapter(nn.Module):
    def __init__(self, D_features, mlp_ratio=0.25, act_layer=nn.GELU, skip_connect=True):
        ...
        self.norm = nn.BatchNorm1d(D_features)
...
class Decoder_Block(nn.Module):
    def __init__(self,
        ...
        # self.with_cls_token = kwargs['decoder_cls_token']

        self.norm1 = norm_layer(dim_in)
        self.attn = Decoder_Attention(
            dim_in, dim_out, num_heads, qkv_bias, attn_drop, drop,
            **kwargs
        )
        ...
        self.adapter = Adapter(dim_out)  # with skip connection
    ...
    def forward(self, x, f):
        ...
        # x = res + self.drop_path(attn)
        # x = x + self.drop_path(self.mlp(self.norm2(x)))
        x = res + self.adapter(self.drop_path(attn))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class Attention_Block(nn.Module):
    def __init__(self, channel_num, fuse_num):
        super(Attention_Block, self).__init__()
        self.attn_norm1_feature = LayerNorm(channel_num,eps=1e-6)

        self.softmax = nn.Softmax(dim=3)
        self.attn_norm = LayerNorm(channel_num * fuse_num, eps=1e-6)
        self.KV_chanel = channel_num * fuse_num
        ...
        self.query1 = nn.Linear(channel_num, channel_num, bias=False)
        self.query2 = nn.Linear(channel_num, channel_num, bias=False)
        self.query3 = nn.Linear(channel_num, channel_num, bias=False)


        self.out = nn.Linear(channel_num, channel_num, bias=False)
        self.out1 = nn.Linear(channel_num, channel_num, bias=False)
        self.out2 = nn.Linear(channel_num, channel_num, bias=False)
        self.out3 = nn.Linear(channel_num, channel_num, bias=False)


        self.key = nn.Linear(channel_num * fuse_num, channel_num * fuse_num, bias=False)
        self.value = nn.Linear(channel_num * fuse_num, channel_num * fuse_num, bias=False)
    ...
    def forward(self, multi_features):
        ...
        feature1 = multi_features[0]
        feature2 = multi_features[1]  # (2, 2048, 32)
        feature3 = multi_features[2]

        all_feature = torch.cat(multi_features, dim=2)  # (2.2048, 96)

        em_feature1 = self.attn_norm1_feature(feature1)
        em_feature2 = self.attn_norm1_feature(feature2)
        em_feature3 = self.attn_norm1_feature(feature3)
        em_all_feature = self.attn_norm(all_feature)

        Q1 = self.query1(em_feature1)
        Q2 = self.query2(em_feature2)
        Q3 = self.query1(em_feature3)

        K = self.key(em_all_feature)
        V = self.value(em_all_feature).transpose(-1, -2)

        attention_scores1 = torch.matmul(Q1.transpose(-1, -2), K)/ math.sqrt(self.KV_chanel)
        attention_scores2 = torch.matmul(Q2.transpose(-1, -2), K)/ math.sqrt(self.KV_chanel)
        attention_scores3 = torch.matmul(Q3.transpose(-1, -2), K)/ math.sqrt(self.KV_chanel)
        ...
        out_feature1 = torch.matmul(attention_probs1, V).transpose(-1, -2)   # （2,32,2048）
        out_feature2 = torch.matmul(attention_probs2, V).transpose(-1, -2)
        out_feature3 = torch.matmul(attention_probs3, V).transpose(-1, -2)

        all_feature = self.proj_dropout(self.out(out_feature1 + out_feature2 + out_feature3))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.att_fuse = Attention_Block(channel_num=embed_dim, fuse_num=3)
    ...
    def forward(self, x):
        ...
        multi_features_res = torch.zeros_like(x)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i >= self.depth-3:
                new_feature = x
                now_cls_tokens, now_x = torch.split(new_feature, [1, F], 1)
                multi_features_res += now_x

                multi_features.append(now_x)
        ...
        att_features = self.att_fuse(multi_features)


        # all_features = att_features
        all_features = att_features + multi_features_res


        for i, blk in enumerate(self.decoder_blocks):
            all_features = blk(all_features, F)
        all_features = rearrange(all_features, 'b f c -> b c f')


        # all_features = rearrange(x, 'b f c -> b c f')

        # print(f"after:{multi_features.shape}")
        # import sys
        # sys.exit()
        ...
        return all_features, cls_tokens, cls_prob, scores, contrast_pairs, embeddings
